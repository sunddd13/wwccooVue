<template>
  ...
  <div class="chat-conversation">
    ...
    <div class="container p-lg-4">
      ...
      <br><br>
      <span class="span-0">
        <Icon icon="mdi:lightbulb-help" /> p-fy 翻译发送的内容(自适应语言),例如：p-fy-你好。
      </span>
      <br><br>
      <span class="span-0">
        <Icon icon="material-symbols:tips-and-updates-rounded" />
        当前Free库数:{{ gptDataSum.freeDataSum }},可用:{{ gptDataSum.freeDataIdleSum }}。Vip库数:{{ gptDataSum.vipDataSum }},可用:{{
          gptDataSum.vipDataIdleSum }}。
      </span>

      <br><br>
      <li class="btn btn-light btn-sm" onclick="window.open('https://afdian.net/a/wwccoo', '_blank')">#前往爱发电支持作者</li>
      <br><br>
      <li class="btn btn-light btn-sm" onclick="window.open('https://docs.qq.com/doc/DYURFQUlZY1dIc3Bm', '_blank')">说明与介绍
      </li>
          ...
          <div class="conversation-list-right">
            <div class="ctext-wrap">
              <div class="ctext-wrap-content">
                <MdEditor class="mychatMd" preview-only v-model="item.sendChatData"></MdEditor>
              </div>
            </div>
          </div>
        </li>
              ...
              <div class="ctext-wrap-content">
                ...
                <template v-if="item.roboTaskData.fanyi == false">
                  <MdEditor class="robotMd" v-model="item.roboTaskData.data" preview-only
                    :class="item.code === 0 ? '' : 'robotMd-redColor'"></MdEditor>
                </template><template v-else>
                ...
                <template v-if=!item.roboTaskData.end_turn>
                  ...
                  <p class="chat-time mb-0">
                    <button class="chat-button" @click="fanyiRobotData(index)">
                      <Icon icon="heroicons-solid:translate" />{{ item.roboTaskData.fanyi ? '原文' : '翻译' }}
                    </button>
                    <button class="chat-button" @click="copyRobotData(index)">
                      <Icon icon="icon-park-twotone:copy-one" />复制
                    </button>
                  </p>
  ...
  <template v-if="openUserChatMod.state.open_chatId_conversation_id == ''">
    <div class="chat-1" :style="{ 'height': inputHeight + 9 + 'px' }">
      <textarea type="text" :style="{ 'height': inputHeight + 'px' }" maxlength="10000" placeholder="ctrl+enter快捷发送。"
        v-model="inputMessage" @keydown.ctrl.enter="sendMessage"></textarea>
      <button class="button-1" @click="themeClick">
        <template v-if="theme.state.data">
          <Icon icon="ph:moon" :color="theme.state.data ? '#36404a' : ''" />
        </template><template v-else>
    ...
    <div class="chat-2">
      <button class="btn btn-primary btn-block left" @click="openChatDataId"
        :disabled="openChatDataButtonCode">拉取本次会话</button>
    </div>
  </template>
...
const chatSendToSer = async (msg: string) => {
  ...
  postData("robotAsk", {
    ...
    .catch((err: any) => {
      /* 不清空 连续对话的 参数 */
      chatContent.state[chatContent.state.length - 1].code = -1
      chatContent.state[chatContent.state.length - 1].roboTaskData.data = "网络\n访问\n异常。";
      chatContent.state[chatContent.state.length - 1].roboTaskData.end_turn = true
      sendDisbled.value = false;
      scrollDown()
    });
...
.chat-1 .button-1:hover {
  background-color: var(--chat1-button-background);
  /* 焦点时添加黑色边框 */
}
